...
public class CustodyLogService {
    ...
    @Transactional(readOnly = true)
    public List<CustodyLog> getRecentLogs() {
        return custodyLogRepository.findTop20ByOrderByTimestampDesc();
    }

    @Transactional(readOnly = true)
    public List<CustodyLog> getSuspiciousLogs() {
        return custodyLogRepository.findBySuspiciousTrueOrderByTimestampDesc();
    }

    @Transactional(readOnly = true)
    public List<CustodyLog> getLogsForEvidence(String evidenceId) {
        return custodyLogRepository.findByEvidenceEvidenceIdOrderByTimestampDesc(evidenceId);
    }
}
